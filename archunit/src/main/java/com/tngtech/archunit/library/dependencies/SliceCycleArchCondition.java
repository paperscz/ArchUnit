...
class SliceCycleArchCondition extends ArchCondition<Slice> {
    ...
    private static class EventRecorder {
        ...
        private List<String> dependenciesDescription(Edge<Slice, Dependency> edge) {
            ...
            List<Dependency> limitedDependencies = allDependencies.size() > maxDependencies
                    ? allDependencies.subList(0, maxDependencies)
                    : allDependencies;
            for (Dependency dependency : limitedDependencies) {
                result.add(dependency.getDescription());
            }
            if (limitedDependencies.size() != allDependencies.size()) {
                result.add(String.format("(%d further dependencies have been omitted...)",
                        allDependencies.size() - limitedDependencies.size()));
            }
            return result;
        }
    }
